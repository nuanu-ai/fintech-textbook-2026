...
В evidence указан источник, snapshot и точный раздел; полные прямые URL, реальный объём чтения, метод доступа и права находятся в [Источники](Источники.md). `supports` означает поддержку тезиса, `context` — основание для явно авторского вывода, `contradicts` — опровержение сформулированного мифа. Упрощённые расчёты главы — синтетические сценарии, а не эмпирические данные провайдеров.
