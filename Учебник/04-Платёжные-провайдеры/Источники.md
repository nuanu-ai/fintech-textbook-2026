...
Ниже приведены 24 реально прочитанных источника. Неполученные кандидаты вынесены в [RESEARCH_NOTES.md](Заметки-исследования.md) и не входят в число прочитанных доказательств. `source_id` постоянен в этой части; `snapshot_id = source_id-20260915` означает дату обращения, а не архивный файл.
